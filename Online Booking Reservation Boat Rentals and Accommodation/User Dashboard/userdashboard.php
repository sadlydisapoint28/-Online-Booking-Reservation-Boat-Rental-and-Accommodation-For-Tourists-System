...
if (!$auth->isLoggedIn()) {
    header("Location: ../php/pages/login/login.php");
    exit();
}
...
$stmt = $pdo->prepare("
    SELECT b.*, bt.name as boat_name, bt.image_url, bt.price, bt.capacity 
    FROM bookings b 
...
$stmt = $pdo->prepare("
    SELECT b.*, bt.name as boat_name, bt.image_url, bt.price 
    FROM bookings b 
    JOIN boats bt ON b.boat_id = bt.id 
    WHERE b.user_id = ? AND b.date < CURDATE()
    ...
    $calendar_events[] = [
        ...
        'className' => $booking['status'] === 'completed' ? 'bg-green-500' : 
                      ($booking['status'] === 'pending' ? 'bg-yellow-500' : 
                       ($booking['status'] === 'active' ? 'bg-blue-500' : 'bg-red-500'))
    ];
}
